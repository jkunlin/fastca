...
public:
  ...
  double
  nextHalfOpen();      // generates random float in [0..1), 2^53 possible values
...
/*
  ...
  2. Seeding with an array (die-hard seed method)
     The length of the array, len, can be arbitrarily high, but for lengths
  greater
     than N, collisions are common. If the seed is of high quality, using more
  than
